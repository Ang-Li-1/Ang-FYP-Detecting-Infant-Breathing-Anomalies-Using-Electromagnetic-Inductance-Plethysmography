...
// 60-120s   Left-dominant   -> ACTIVE
// 120-180s  Right-dominant  -> STOP
// 180-240s  Apnea           -> STOP
//
...
bool isActiveLeft(unsigned long tInCycle) {
  // Left ACTIVE during Balanced + Left-dominant
  // STOP during Right-dominant + Apnea
  if (tInCycle < 60000UL)  return true;   // Balanced
  if (tInCycle < 120000UL) return true;   // Left-dominant
  if (tInCycle < 180000UL) return false;  // Right-dominant
  return false;                           // Apnea
}
...
void setup() {
  ...
  Serial.begin(9600);
  Serial.println("LEFT pump started");
...
void loop() {
  ...
  bool active = isActiveLeft(tInCycle);

  // Detect STOP -> ACTIVE transition: trigger residual
  if (active && !lastActive) {
    phase = PH_RESIDUAL;
    phaseStart = now;
    Serial.print("[LEFT] STOP->ACTIVE, residual. mode=");
  ...
  if (!active && lastActive) {
    phase = PH_STOP;
    phaseStart = now;
    Serial.print("[LEFT] ACTIVE->STOP. mode=");
    Serial.println(modeName(modeIdx));
    allStop();
  }
