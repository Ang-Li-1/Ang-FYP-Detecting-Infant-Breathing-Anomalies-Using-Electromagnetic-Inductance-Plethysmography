...
# Adapted for new UDP-sync readout strategy:
# - ESP32 may skip suspicious samples.
# - Python only processes when both channels have new samples.
# - Python does NOT duplicate the last value to maintain 20 Hz.
# =========================================================

# ===================== Network =====================
ESP32_RIGHTLUNG_IP = "10.104.139.110"
ESP32_LEFTLUNG_IP  = "10.104.139.130"
PORT = 3333
...
# Faster predicted-class update:
# Still use a 5 s sliding feature window, but update prediction every 1 s.
...
# Because classification now updates every 1 s, 18 windows means about 18 s.
# Original value is kept to preserve your logic structure.
MIN_WINDOWS_FOR_GMM = 18

# Refit every 3 prediction updates, now about every 3 s.
GMM_REFIT_EVERY = 3
...
try:
    ...
    print("[Info] Adapted for UDP-sync readout: Python only processes paired new samples.")
    print("[Info] Faster prediction update enabled.")
    print("[Info] Feature window = 5 s, predicted class update = 1 s.")
    print("[Info] GMM enable after MIN_WINDOWS_FOR_GMM prediction windows.")
    print(f"[Info] Window features use {WINDOW_SEC:.0f}s; display/log update every {WINDOW_UPDATE_SEC:.1f}s.")
    ...
    # Latest accepted sample times from each ESP32.
    # Used to reject duplicate samples.
    last_seen_t_r = None
    last_seen_t_l = None

    # Pending new samples waiting to be paired.
    pending_r = None   # tuple: (t_us, adc)
    pending_l = None   # tuple: (t_us, adc)
    ...
    while True:
        ...
        try:
            if sock_right in rlist:
                ...
                if sr is not None:
                    ...
                    # Accept only genuinely new rightlung sample
            ...
            if sock_left in rlist:
                ...
                if sl is not None:
                    ...
                    # Accept only genuinely new leftlung sample
        ...
        # Do NOT repeat the previous value if one ESP32 skipped a sample.
        # =========================================================
        ...
        # ===================== Original signal processing logic =====================
        ...
        if len(win_right) == WINDOW_SAMPLES and len(win_left) == WINDOW_SAMPLES:
            if (now - last_window_emit_time) >= WINDOW_UPDATE_SEC:
                ...
                info_text.set_text(
                    f"Breathing rate={br_str}   "
                    f"A_leftlung={Al_str}   "
                    f"A_rightlung={Ar_str}   "
                    f"D={D_str}   "
                    f"Class={label_smooth}   "
